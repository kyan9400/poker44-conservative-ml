...
class MLScorer:
    ...
    def _predict_proba_batch(self, X: np.ndarray) -> np.ndarray:
        """Return per-row P(bot) for a stacked feature matrix.

        Avoids the per-row Python loop overhead of `_predict_proba_vector`
        when scoring many chunks (e.g. 40-chunk validator batch). For
        `ensemble_models`, each model is invoked once on the full matrix
        ...
        if bundle.get("ensemble_models"):
            ...
            for model in bundle["ensemble_models"]:
                if hasattr(model, "predict_proba"):
                    stacks.append(np.asarray(model.predict_proba(X)[:, 1], dtype=np.float64))
                else:
                    stacks.append(np.asarray(model.predict(X), dtype=np.float64))
            return np.clip(np.mean(np.stack(stacks, axis=0), axis=0), 0.0, 1.0)
        model = bundle["model"]
        if hasattr(model, "predict_proba"):
            return np.clip(np.asarray(model.predict_proba(X)[:, 1], dtype=np.float64), 0.0, 1.0)
    ...
    def predict_proba_chunks_batch(self, chunks: List[List[dict]]) -> np.ndarray:
        """Score a list of chunks in one shot (only chunk-granularity models).

        Returns an array of P(bot) per chunk. Falls back to per-chunk path
        for hand-granularity models (where the chunk -> hands fan-out makes
        a single matrix call ill-defined).
        """
        if not self._bundle:
            self.load()
        if not chunks:
        ...
        if self.granularity == "hand":
            return np.asarray([self.predict_proba_chunk(c) for c in chunks], dtype=np.float64)
        rows = [self._vectorize(c)[0] for c in chunks]
    ...
    def score_chunks(
        ...
        *,
        ...
        ml_probs: List[float] = []
        # Batched fast path for chunk-granularity models (incl. ensemble).
        if self.granularity == "chunk":
            try:
                arr = self.predict_proba_chunks_batch(visible_chunks)
                ml_probs = [float(x) for x in arr]
            except Exception:
                ml_probs = []
        if not ml_probs:
            for visible, h in zip(visible_chunks, heur):
                try:
